...
class TuyaBLEDevice:
    def __init__(
        ...
        # Protocol generation (GATT chars + device-info payload). Defaults to v4;
        # set_generation() overrides it from the advertised service at connect.
    ...
    def set_generation(self, generation) -> None:
        """Override the protocol generation (GATT chars + device-info payload)."""
    ...
    async def _execute_disconnect(self) -> None:
        ...
        async with self._connect_lock:
            ...
            if client and client.is_connected:
                await client.stop_notify(self._generation.char_notify)
                ...
                if client and client.is_connected:
                    ...
                    try:
                        await self._client.start_notify(
                            self._generation.char_notify, self._notification_handler
                        )
                ...
                if self._client and self._client.is_connected:
                    ...
                    try:
                        if not await self._send_packet_while_connected(
                            ...
                            # Generation-specific: v4 requires [0x00, 0xF3] or the
                            # board drops the link; v3 uses an empty payload.
                            self._generation.device_info_payload,
                            0,
                            True,
                        ):
                            self._client = None
    ...
    def _parse_datapoints_v4(
        ...
        # LOCAL MOD (v4 / 0xFD50 board): v4 DP reports carry a 7-byte header
        # (sn:4, s_type:1, s_mode:1, s_ack:1) then datapoints whose length
        # field is 2 bytes big-endian (vs 1 byte in v3). (ha_tuya_ble#160)
        datapoints: list[TuyaBLEDataPoint] = []
        pos = start_pos
        while len(data) - pos >= 4:
            id: int = data[pos]
            ...
            match type:
                ...
                case TuyaBLEDataPointType.DT_STRING:
                    value = raw_value.decode()
            _LOGGER.debug(
                "%s: Received v4 datapoint, id: %s, type: %s, value: %s",
                self.address, id, type.name, value,
            )
    ...
    def _handle_command_or_response(
        ...
        match code:
            ...
            case TuyaBLECode.FUN_RECEIVE_DP_V4:
                # LOCAL MOD: parse v4 DP report (7-byte header, 2-byte lengths).
                # No ack sent for now — a one-shot read drains the board's cache
                # within the settle window; proper v4 acks are a later concern.
